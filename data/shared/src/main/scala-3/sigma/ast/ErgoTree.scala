...
object ErgoTree {
  ...
  opaque type HeaderType = Byte
  
  object HeaderType {
    def apply(b: Byte): HeaderType = b
  }
  ...
  val ZeroHeader: HeaderType = 0.toByte
  ...
  val ConstantSegregationHeader: HeaderType = (DefaultHeader | ConstantSegregationFlag).toByte
  ...
  @inline final def setRequiredBits(header: HeaderType): HeaderType = {
    if (getVersion(header) > 0) {
      ...
      (header | ErgoTree.SizeFlag).toByte
    } else
      header
  }
  ...
  @inline final def setConstantSegregation(header: HeaderType): HeaderType = {
    (header | ConstantSegregationFlag).toByte
  }

  /** Sets the ConstantSegregationFlag in the given header */
  @inline final def setSizeBit(header: HeaderType): HeaderType = {
    (header | SizeFlag).toByte
  }
