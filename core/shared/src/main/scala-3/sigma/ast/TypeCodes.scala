...
object TypeCodes {
  opaque type TypeCode = Byte
  
  object TypeCode {
    def apply(b: Byte): TypeCode = b
  }
  ...
  val FirstDataType: TypeCode = 1.toByte

  val LastDataType : TypeCode = 111.toByte

  /** SFunc type */
  val FuncType: TypeCode = (LastDataType + 1).toByte
